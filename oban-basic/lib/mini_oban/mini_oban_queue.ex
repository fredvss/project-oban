defmodule MiniOban.Queue do
  ...
  def handle_cast({:queue_job, job}, state) do
    Bunt.puts([:cyan, "[Queue] Queued job #{job.id} | type=#{job.type}"])
    ...
    if slots <= 0 do
      ...
      new_running =
        Enum.reduce(to_run, state.running, fn (job, running) ->
          ...
          Bunt.puts([:yellow, "[Queue] Dispatching job #{job.id} | running=#{map_size(running) + 1}/#{state.max_concurrency}"])
  ...
  def handle_info({:requeue, job}, state) do
    Bunt.puts([:yellow, "[Queue] Requeueing job #{job.id} after backoff"])
    new_state = %{state | pending: state.pending ++ [job]}
    send(self(), :dispatch)
    {:noreply, new_state}
  end
  ...
  def handle_info({ref, result}, state) when is_map_key(state.running, ref) do
    ...
    new_state =
      case result do
        :ok ->
          Bunt.puts([:green, "[Queue] Job #{job.id} succeeded | attempt=#{job.attempts + 1} | duration=#{duration_ms}ms"])
        ...
        {:error, reason} ->
          if job.attempts + 1 < job.max_attempts do
            ...
            Bunt.puts([:magenta, "[Queue] Job #{job.id} failed (#{reason}) — retrying in #{delay_ms}ms (attempt #{retry_job.attempts}/#{retry_job.max_attempts})"])
            Process.send_after(self(), {:requeue, retry_job}, delay_ms)
            %{state | running: new_running}
          else
            Bunt.puts([:red, "[Queue] Job #{job.id} permanently failed after #{job.attempts + 1} attempt(s)"])
  ...
  def handle_info({:DOWN, ref, :process, _, reason}, state) when is_map_key(state.running, ref) do
    ...
    new_state =
      if job.attempts + 1 < job.max_attempts do
        retry_job = %{job | attempts: job.attempts + 1, status: :pending, started_at: nil}
        delay_ms = retry_job.attempts * 1_000
        Bunt.puts([:magenta, "[Queue] Job #{job.id} crashed (#{inspect(reason)}) — retrying in #{delay_ms}ms (attempt #{retry_job.attempts}/#{retry_job.max_attempts})"])
        Process.send_after(self(), {:requeue, retry_job}, delay_ms)
        %{state | running: new_running}
      else
        Bunt.puts([:red, "[Queue] Job #{job.id} crashed permanently after #{job.attempts + 1} attempt(s)"])
        finished_job = %{job | status: :failed, finished_at: finished_at}
        %{state | running: new_running, completed: state.completed ++ [finished_job]}
      end
