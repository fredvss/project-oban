defmodule MiniOban.Queue do
  ...
  def handle_info({:requeue, job}, state) do
    Bunt.puts([:yellow, "[Queue] Requeueing job #{job.id} after exponential backoff"])
    new_state = %{state | pending: state.pending ++ [job]}
    send(self(), :dispatch)
    {:noreply, new_state}
  end
      ...
      case result do
        ...
        {:error, reason} ->
          ...
          if next_attempt < job.max_attempts do
            ...
            Bunt.puts([:magenta, "[Queue] Job #{job.id} failed (#{reason}) — retrying in #{delay_ms}ms (attempt #{next_attempt}/#{retry_job.max_attempts})"])
            Process.send_after(self(), {:requeue, retry_job}, delay_ms)
            %{state | running: new_running}
          else
            Bunt.puts([:red, "[Queue] Job #{job.id} permanently failed after #{next_attempt} attempt(s)"])
  ...
  def handle_info({:DOWN, ref, :process, _, reason}, state) when is_map_key(state.running, ref) do
    ...
    new_state =
      if next_attempt < job.max_attempts do
        ...
        delay_ms = trunc(:math.pow(2, next_attempt) * 1_000)
        Bunt.puts([:magenta, "[Queue] Job #{job.id} crashed (#{inspect(reason)}) — retrying in #{delay_ms}ms (attempt #{next_attempt}/#{retry_job.max_attempts})"])
        Process.send_after(self(), {:requeue, retry_job}, delay_ms)
        %{state | running: new_running}
      else
        Bunt.puts([:red, "[Queue] Job #{job.id} crashed permanently after #{next_attempt} attempt(s)"])
        finished_job = %{job | status: :failed, finished_at: finished_at}
        %{state | running: new_running, completed: state.completed ++ [finished_job]}
      end
